        ...
        <tbody>
          @foreach ($refunds as $refund)
            <tr>
              ...
              <td>{{ get_formated_currency($refund->order->grand_total, 2, config('system_settings.currency.id')) }}</td>
              <td>{{ get_formated_currency($refund->amount, 2, config('system_settings.currency.id')) }}</td>
        ...
        <tbody>
          @foreach ($closed as $refund)
            <tr>
              ...
              <td>{!! get_yes_or_no($refund->return_goods) !!}</td>
              <td>{{ get_formated_currency($refund->order->total, 2, config('system_settings.currency.id')) }}</td>
              <td>{{ get_formated_currency($refund->amount, 2, config('system_settings.currency.id')) }}</td>
              <td>{!! $refund->statusName() !!}</td>
              <td>{{ $refund->created_at->diffForHumans() }}</td>
              <td>{{ $refund->updated_at->diffForHumans() }}</td>
