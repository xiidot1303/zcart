...
class demoSeeder extends BaseSeeder
{
    ...
    public function run()
    {
        ...
        foreach ($categories as $catId) {
            foreach (array_rand($attributes, rand(2, 5)) as $key) {
                $attribute_categories[] = [
        ...
        $attribute_inventory = [];
        foreach ($categories as $catId) {
            $catAttrs = DB::table('attribute_categories')
                ->where('category_id', $catId)->pluck('attribute_id')->toArray();

            foreach (array_unique($catAttrs) as $attrId) {
                $attrVals = DB::table('attribute_values')
                    ->where('attribute_id', $attrId)->pluck('id')->toArray();

                if (empty($attrVals)) continue;

                $attribute_inventory[] = [
                    'attribute_id' => $attrId,
                    'inventory_id' => $inventories_ids[array_rand($inventories_ids)],
                    'attribute_value_id' => $attrVals[array_rand($attrVals)],
                    'created_at' => $this->now,
                    'updated_at' => $this->now,
                ];
            }
        ...
        $trending_keywords = [
            'aperiam', 'iste', 'aut', 'porro', 'iPhone 13', 'Galuxy S', 'omnis', 'provident'
        ];
        ...
        DB::table('options')->insert([
            [
                ...
                'option_name' => 'trending_keywords',
                'option_value' => serialize($trending_keywords),
                'autoload' => 1,
                'created_at' => $this->now,
                'updated_at' => $this->now,
            ], [
