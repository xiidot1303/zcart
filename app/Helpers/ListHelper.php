...
class ListHelper
{
    public static function common_select_attr($model = 'inventory')
    {
        switch ($model) {
            case 'inventory':
                return ['inventories.id', 'inventories.shop_id', 'title', 'inventories.product_id', 'sku', 'condition', 'stock_quantity', 'min_order_quantity', 'sold_quantity', 'available_from', 'sale_price', 'offer_price', 'offer_start', 'offer_end', 'free_shipping', 'slug', 'stuff_pick'];
        }
    ...
    public static function best_find_under($price, $limit = 20, $shop_id = null)
    {
        ...
        if ($shop_id) {
            $items = $items->where('shop_id', $shop_id)->groupBy('product_id');
        } else {
            $items = $items->groupBy('product_id', 'shop_id');
        }
