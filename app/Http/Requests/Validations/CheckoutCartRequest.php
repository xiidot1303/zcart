    ...
    public function rules()
    {
        ...
        if (!$this->checkAuth()) {
            ...
            if (!isset($customer) && $this->has('create-account')) {
                $email_rules .= '|unique:customers';
                // $password_rules .= '|required_with:create-account|confirmed';
                $rules['password'] = 'required|confirmed|min:6';
            }

            $rules['email'] = $email_rules;
            // $rules['password'] = $password_rules;
            // $rules['address_title'] = 'required|regex:/^[a-zA-Z]+ [a-zA-Z]+$/';
            $rules['address_title'] = 'required|regex:/^\w+\s+\w+/';
        }
