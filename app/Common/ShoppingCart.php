...
trait ShoppingCart
{
    ...
    private function getShoppingCarts($from_api = false)
    {
        if ($from_api) {
            ...
            $carts = Cart::whereNull('customer_id')->whereIn('id', Session::get('session_carts'));
        }
    ...
    public function addToCart(Request $request, $slug)
    {
        ...
        if ($customer_id) {
            ...
            if (!empty(Session::get('session_carts'))) {
                $old_cart = $old_cart->orWhere(function ($q) {
                    $q->whereNull('customer_id')
                        ->whereIn('id', Session::get('session_carts'));
                });
            }
        } elseif (!empty(Session::get('session_carts'))) {
        ...
        // Push the card id to session if not exist
        if (!in_array($cart->id, Session::get('session_carts'))) {
            Session::push('session_carts', $cart->id);
        }
